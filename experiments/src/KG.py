...
# example: python KG.py GP 1 1000 0 1
argv = sys.argv[1:]
...
cpp_sgd_params_ps = cppGradientDescentParameters(num_multistarts=1, max_num_steps=12, max_num_restarts=1,
...
for job_no in xrange(start_idx, end_idx):
    ...
    init_points = python_search_domain.generate_uniform_random_points_in_domain(100)
    temp_mean=numpy.zeros(100)
    temp_std=numpy.zeros(100)
    with PdfPages('../Plots/KG.pdf') as pdf:
        for n in xrange(num_iteration):
            ...
            pd.DataFrame(numpy.concatenate((temp_mean.reshape(100, 1), temp_std.reshape(100, 1)), axis=1)).to_csv("../Plots/KG/post"+str(n)+".csv")
            ...
            cpp_kg_evaluator_nogradient = cppKnowledgeGradient(gaussian_process=cpp_gp_nogradient, num_fidelity=0,
                                                               inner_optimizer = ps_sgd_optimizer,
                                                               discrete_pts=init_points, num_mc_iterations=100)
            ...
            for i in xrange(len(xlist)):
                cpp_kg_evaluator_nogradient.set_current_point(xlist[i])
                Y[i] = cpp_kg_evaluator_nogradient.compute_objective_function()
                if Y[i]>kgmax:
                    kgmax=Y[i]
                    next_point_kg=xlist[i]

            plt.plot(discrete_pts.flatten(), Y, 'g', linestyle = '-.')
            plt.plot(next_point_kg, kgmax, 'rs')
            plt.legend(['KG'], fontsize=20, loc='upper left')

            #plt.legend(['mc sample 20', 'mc sample 100', 'mc sample 1000'], fontsize=20, loc='upper left')
            pd.DataFrame(numpy.concatenate((discrete_pts, numpy.array(Y).reshape(100, 1)), axis=1)).to_csv("../Plots/KG/kg"+str(n)+".csv")
            #with PdfPages('../Plots/Tutorial/KG'+str(n)+'.pdf') as pdf:
            pdf.savefig()  # saves the current figure into a pdf page
            plt.close()

            sampled_points = [SamplePoint(pt, objective_func.evaluate(pt)[0], objective_func._sample_var) for pt in [[next_point_kg]]]
